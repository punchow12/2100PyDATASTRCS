...
class BoundedPriorityQueue:
    ...
    def add(self, item: any, priority: int) -> None:
        ...
        if priority > num_levels - 1 or priority < 0:
            raise ValueError(
                "add(item, priority):" + str(priority) + " is an invalid priority level")

        # Adds item to its level
    ...
    def remove(self) -> any:
        ...
        # Iterates through all the levels in the queue
        for level in self._queues:
            num_items_in_level = 0

            # Finds the first level with items inside
            for item in level:
                num_items_in_level += 1

            # Removes and returns first item in level
