...
#         return 1 + max(self._height(node.left), self._height(node.right))
    ...
    def _inorder_print(self, node: 'BinaryTree._Node') -> None:
        """Print the binary tree rooted at node using a inorder traversal."""
        if node is None:
            return
   ...
   # Exercise 7
    ...
    def count(self, item: any) -> int:
        ...
        def _count(node: 'BinaryTree._Node') -> int:
            """Return the number of times item occurs in the tree rooted at node."""
            x = 0
            if node is None:
                return 0
            if node.x is item:
                x = 1
            return x + _count(node.left) + _count(node.right)
...
def build_binary_tree() -> BinaryTree:
    ...
    return tree  # returns a weird looking list that honestly tells nothing
...
def build_perfect_binary_tree() -> BinaryTree:
    ...
    and return the reference to the tree.
    """
    list = [5, 7, 12, 17, 9, 3, 6]
    x = 0
    tree = BinaryTree()
    rootnode = BinaryTree._Node(list[x])
    tree._root = rootnode
    # Insert left child of root.
    rootnode.left = BinaryTree._Node(list[x + 1])
    # Insert right child of root.
    rootnode.right = BinaryTree._Node(list[x + 2])

    rootnode.left.left = BinaryTree._Node(list[x + 3])
    rootnode.left.right = BinaryTree._Node(list[x + 4])

    rootnode.right.left = BinaryTree._Node(list[x + 5])
    rootnode.right.right = BinaryTree._Node(list[x + 6])
